...
public class StreamWikiDumpInputFormat extends KeyValueTextInputFormat {
  ...
  private static final String KEY_EXCLUDE_PAGE_PATTERN = "org.wikimedia.wikihadoop.excludePagesWith";
	...
	public InputSplit[] getSplits(JobConf job, int numSplits)
			throws IOException {
		LOG.info("StreamWikiDumpInputFormat.getSplits job=" + job + " n="
				+ numSplits);
		...
		for (FileStatus file : files) {
			...
			LOG.info(String.format("goalsize=%d splitsize=%d blocksize=%d",
					goalSize, splitSize, blockSize));
			// System.err.println(String.format("goalsize=%d splitsize=%d blocksize=%d",
			// goalSize, splitSize, blockSize));
			for (InputSplit x : getSplits(job, file, pageBeginPattern,
					splitSize))
				splits.add(x);
		}
	...
	private FileSplit makeSplit(Path path, long start, long size,
			NetworkTopology clusterMap, BlockLocation[] blkLocations)
			throws IOException {
		// return new FileSplit(path, start, size, getSplitHosts(blkLocations,
		// start, size, clusterMap));
	...
	public List<InputSplit> getSplits(JobConf job, FileStatus file,
			String pattern, long splitSize) throws IOException {
		...
		if ((length != 0) && isSplitable(fs, path)) {
			...
			SeekableInputStream in = SeekableInputStream.getInstance(path, 0,
					length, fs, this.compressionCodecs);
			InputStream is = null;
			long start = 0;
			long skip = 0;
			if (is != null) {
			...
			READLOOP: while (((double) bytesRemaining) / splitSize > factor
					&& bytesRemaining > 0) {
				// prepare matcher
				ByteMatcher matcher;
				{
					long st = Math.min(start + skip + splitSize, length - 1);
					split = makeSplit(path, st,
							Math.min(splitSize, length - st), clusterMap,
							blkLocations);
					...
					in = SeekableInputStream.getInstance(split, fs,
				...
				boolean reach = false;
				while (!matcher.readUntilMatch(pageEndPattern, null,
						split.getStart() + split.getLength())) {
					if (matcher.getPos() >= length
							|| split.getLength() == length - split.getStart())
						break READLOOP;
					reach = false;
					split = makeSplit(
							path,
							split.getStart(),
							Math.min(split.getLength() + splitSize, length
									- split.getStart()), clusterMap,
							blkLocations);
					System.err.println("split extend to: " + split);
				}
				System.err.println(path + ": #" + splits.size() + " "
						+ pageEndPattern + " found: pos=" + matcher.getPos()
						+ " last=" + matcher.getLastUnmatchPos() + " read="
						+ matcher.getReadBytes() + " current=" + start
						+ " remaining=" + bytesRemaining + " split=" + split);
				if (matcher.getLastUnmatchPos() > 0
					...
					long newstart = Math
							.max(matcher.getLastUnmatchPos(), start);
			...
			if (bytesRemaining > 0 && !processedPageEnds.contains(length)) {
				System.err.println(pageEndPattern + " remaining: pos="
						+ (length - bytesRemaining) + " end=" + length);
	...
	public RecordReader<Text, Text> getRecordReader(
		...
		return new MyRecordReader(split, reporter, job, fs, patt != null
				&& !"".equals(patt) ? Pattern.compile(patt) : null, prev);
	}

	private class MyRecordReader implements RecordReader<Text, Text> {

		public MyRecordReader(FileSplit split, Reporter reporter, JobConf job,
				FileSystem fs, Pattern exclude, boolean prev)
				throws IOException {
			...
			this.revHeader = getBuffer(this.revisionBeginPattern
					.getBytes("UTF-8"));
			...
			SeekableInputStream in = SeekableInputStream.getInstance(split, fs,
					compressionCodecs);
			...
			this.pageBytes = getPageBytes(this.split, this.fs,
					compressionCodecs, this.reporter);

			this.istream = SeekableInputStream.getInstance(this.split, this.fs,
					compressionCodecs);
		...
		public float getProgress() throws IOException {
			...
			if (this.end == this.start) {
				...
				rate = ((float) (this.getPos() - this.start))
		...
		@Override
		synchronized public boolean next(Text key, Text value)
				throws IOException {
			// LOG.info("StreamWikiDumpInputFormat: split=" + split + " start="
			// + this.start + " end=" + this.end + " pos=" + this.getPos());
			...
			while (true) {
				...
				// System.err.println("0.2 check pos="+this.getPos() +
				// " end="+this.end);//!
				if (this.currentPageNum >= this.pageBytes.size() / 2
						|| this.getReadBytes() >= this.tailPageEnd()) {
					return false;
				}

				// System.err.println("2 move to rev from: " +
				// this.getReadBytes());//!
				if (!readUntilMatch(this.revisionBeginPattern,
						this.bufBeforeRev)
						|| this.getReadBytes() >= this.tailPageEnd()) { // move
																		// to
																		// the
																		// beginning
																		// of
				...
				// System.err.println("2.1 move to rev to: " +
				// this.getReadBytes());//!

				// System.err.println("4.5 check if exceed: " +
				// this.getReadBytes() + " " + nextPageBegin() + " " +
				// prevPageEnd());//!
				if (this.getReadBytes() >= this.nextPageBegin()) {
					// int off = (int)(this.nextPageBegin() -
					// this.prevPageEnd());
					int off = findIndex(pageBeginPattern.getBytes("UTF-8"),
							this.bufBeforeRev);
					if (off >= 0) {
						...
						if (this.exclude != null
								&& this.exclude.matcher(
										new String(this.pageHeader.getData(),
												"UTF-8")).find()) {
							reporter.incrCounter(
									WikiDumpCounters.SKIPPED_PAGES, 1);
							this.seekNextRecordBoundary();
						} else {
							reporter.incrCounter(
									WikiDumpCounters.WRITTEN_PAGES, 1);
							break;
						}
			...
			if (!readUntilMatch(this.revisionEndPattern, this.bufInRev)) { // store
																			// the
																			// revision
				// System.err.println("no revision end" + this.getReadBytes() +
				// " " + this.end);//!
			...
			byte[] record = this.recordPrevRevision ? writeInSequence(new DataOutputBuffer[] {
					this.pageHeader, this.prevRevision, this.revHeader,
					this.bufInRev, this.pageFooter })
			...
			value.set("");
			this.reporter
					.setStatus("StreamWikiDumpInputFormat: write new record pos="
							+ this.getPos()
							+ " bytes="
							+ this.getReadBytes()
							+ " next="
		...
		private synchronized boolean readUntilMatch(String textPat,
	...
	private static List<Long> getPageBytes(FileSplit split, FileSystem fs,
			CompressionCodecFactory compressionCodecs, Reporter reporter)
			throws IOException {
		...
		try {
			...
			while (true) {
				if (matcher.getPos() >= end
						|| !matcher.readUntilMatch(pageBeginPattern, null, end)) {
					break;
				}
				ret.add(matcher.getReadBytes()
						- pageBeginPattern.getBytes("UTF-8").length);
				if (matcher.getPos() >= end
						|| !matcher.readUntilMatch(pageEndPattern, null, end)) {
					System.err.println("could not find " + pageEndPattern
							+ ", page over a split?  pos=" + matcher.getPos()
							+ " bytes=" + matcher.getReadBytes());
					// ret.add(end);
					break;
				}
				ret.add(matcher.getReadBytes()
						- pageEndPattern.getBytes("UTF-8").length);
				String report = String
						.format("StreamWikiDumpInputFormat: find page %6d start=%d pos=%d end=%d bytes=%d",
								ret.size(), start, matcher.getPos(), end,
								matcher.getReadBytes());
	...
	private static void offsetWrite(DataOutputBuffer to, int fromOffset,
			DataOutputBuffer from) throws IOException {
		if (from.getLength() <= fromOffset || fromOffset < 0) {
			throw new IllegalArgumentException(String.format(
					"invalid offset: offset=%d length=%d", fromOffset,
					from.getLength()));
		}
	...
	private static void allWrite(DataOutputBuffer to, DataOutputBuffer from)
			throws IOException {
		offsetWrite(to, 0, from);
	}

	private static int findIndex(byte[] match, DataOutputBuffer from_)
			throws IOException {
		...
		System.err.println("pattern not found: " + new String(match) + " in "
				+ new String(from, 0, from_.getLength()));// !
		return -1;
	}
